...
openssl req -new -key legacy-nginx/server.key -out legacy-nginx/server.csr \
...
openssl x509 -req -days 3650 -in legacy-nginx/server.csr -signkey legacy-nginx/server.key -out legacy-nginx/server.crt -sha1

# Clean up the CSR
rm legacy-nginx/server.csr
